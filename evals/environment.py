...
from agent_filetree_memory.postgres import PostgresMemoryStore, PostgresRuntime, PostgresStoreConfig
...
async def environment(files):
    ...
    if not url:
        raise RuntimeError("use python -m devtools.postgres -- uv run --group evals python -m evals.run ...")
    ...
    try:
        ...
        encryptor = EnvelopeEncryptor(LocalKeyringDekProvider(
            {"eval": base64.b64encode(os.urandom(32)).decode()}, active_key_id="eval"))
        store = PostgresMemoryStore(runtime, encryptor, config=PostgresStoreConfig(
            idempotency_index_key=os.urandom(32), rate_limit_operations=10000))
        service = MemoryService(store)
        now = datetime.now(timezone.utc)
        invocation = VerifiedInvocation(
            scope=Scope("eval-workspace", "eval-agent"), principal_id="eval-principal",
            invocation_id="eval-invocation", capability_id="eval-capability",
            issuer="eval", audience="eval", allowed_actions=frozenset(MemoryAction),
            issued_at=now, expires_at=now + timedelta(hours=1))
        for index, (path, content) in enumerate(files.items()):
            await service.write(invocation, path, content, idempotency_key=f"seed-{index}")
