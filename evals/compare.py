...
def compare(before, after):
    ...
    if before["driver"] == "reference":
        print("Reference replay checks fixture/grader plumbing; it does not measure LLM performance.")
    for label, report in (("before", before), ("after", after)):
        rows = report["runs"]
        print(f"{label}: {sum(r['success'] for r in rows)}/{len(rows)} successes; "
    ...
    return 1 if after["case_errors"] or not all(r["success"] for r in after["runs"]) else 0
...
if __name__ == "__main__":
    ...
    raise SystemExit(compare(json.loads(args.before.read_text()), json.loads(args.after.read_text())))
