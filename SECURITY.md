...
The database is not a plaintext content index. Paths and directory manifests are encrypted alongside document bodies. Per-version committer associations, declared co-authors, and change comments are carried inside that encrypted document payload rather than added as plaintext indexes. Each immutable version uses a fresh AES-256-GCM data-encryption key. A configurable provider wraps that key with the same opaque encryption context used as authenticated data for the document ciphertext.
